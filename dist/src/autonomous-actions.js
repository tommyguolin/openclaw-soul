...
import { invokeGatewayTool, isWriteTool } from "./gateway-client.js";
...
const recentReportedMessages = new Map();
function hasRecentProviderPressure(windowMs = PROVIDER_PRESSURE_BACKOFF_MS) {
    const sessionsDir = join(homedir(), ".openclaw/agents/main/sessions");
    const cutoff = Date.now() - windowMs;
    try {
        for (const name of readdirSync(sessionsDir)) {
            if (!name.endsWith(".jsonl") && !name.endsWith(".trajectory.jsonl"))
                continue;
            ...
            for (const line of lines) {
                const providerPressureLine = /"fallbackStepFromFailureReason":"rate_limit"/.test(line) ||
...
async function markCompletedTasksDelivered() {
    await updateEgoStore(resolveEgoStorePath(), (e) => {
        for (const t of e.activeTasks ?? []) {
            if (isReportableTask(t) && !t.resultDelivered)
...
function wantsChineseReport(ego) {
    return ego.userLanguage === "zh-CN" || (ego.recentUserMessages ?? []).some((m) => /[\u4e00-\u9fff]/.test(m));
}
function normalizeTaskResultForReport(result) {
    return result
...
function isProviderPressureErrorText(value) {
...
function isLowValueAutonomousFailure(task) {
    ...
    if (hasContent)
        return false; // has diagnostic content → worth reporting
...
function recentAutonomousFailureBackoff(ego) {
    const now = Date.now();
...
function hasTaskResultFile(task) {
...
function hasFinalTaskResultFile(task) {
    if (!task.resultFilePath)
        return false;
    try {
...
function packageScript(dir, names) {
    try {
...
function verifyAppliedFix(targetDir, fixFile) {
    ...
    // Gradle projects
    try {
        if (statSync(join(targetDir, "build.gradle")).isFile()
    ...
    try {
        ...
        if (statSync(makefile).isFile() || statSync(makefileLower).isFile()) {
            ...
            if (!/No rule.*check/i.test(checkResult.summary))
                return checkResult;
...
function isCompleteTaskReport(result) {
...
function isSubagentSessionRecentlyActive(task) {
    const sessionsDir = join(homedir(), ".openclaw/agents/main/sessions");
    try {
        for (const name of readdirSync(sessionsDir)) {
...
function isTaskBlockedOrPartial(task, result) {
    if (unmetAcceptanceCriteria(task, result).length > 0)
        return true;
...
function taskResultFileLine(task, zh) {
    if (!task.resultFilePath)
        return null;
    try {
...
function taskContinuityFields(thought) {
    ...
    return {
        ...
        ...(Array.isArray(params.acceptanceCriteria)
            ? { acceptanceCriteria: params.acceptanceCriteria.filter((item) => typeof item === "string") }
            : {}),
    };
}
function unmetAcceptanceCriteria(task, result) {
    ...
    const hasChangeEvidence = !explicitlyNoChange && (/修改文件：\s*(?!0\b|无\b|none\b)[^\n]+/i.test(result)
    ...
    const hasVerificationEvidence = !explicitlyNoVerification && (/(?:verification|验证|npm|pnpm|yarn|pytest|typecheck|test|build)[^\n]{0,160}(?:passed|通过|succeeded|成功)/i.test(result)
        || /\b(?:verify|test|typecheck|build)[\w-]*\b/i.test(successfulActions));
    return criteria.filter((criterion) => {
        if (/concrete changed files|files? changed|code change/i.test(criterion))
            return !hasChangeEvidence;
        if (/verification command|verified|verification passes/i.test(criterion))
 ...
 */
export async function executeAutonomousAction(actionType, thought, ego, options) {
    const autoOpts = {
...
// ---------------------------------------------------------------------------
export async function executeInvokeTool(thought, ego, options) {
    const tool = thought.actionParams?.tool ?? "";
    const args = thought.actionParams?.args ?? {};
    if (!tool) {
...
// ---------------------------------------------------------------------------
function buildRuleBasedAnalysis(thought, ego, gatheredInfo, steps) {
    const zh = wantsChineseReport(ego);
...
function isInternalNeedDiagnostic(thought) {
...
function isOnlyProviderPressureDiagnostic(result) {
...
function collectKnownLocalEvidenceTargets(ego, currentText = "") {
    const haystacks = [
        currentText,
...
function isReadableEvidenceFile(filePath) {
    const lower = filePath.toLowerCase();
    return READABLE_EVIDENCE_EXTENSIONS.some((ext) => lower.endsWith(ext));
}
async function readLocalEvidenceTarget(target) {
    const steps = [];
    const evidence = [];
    ...
    try {
        ...
        if (stat.isFile() && isReadableEvidenceFile(target)) {
            ...
            if (step.success && step.output)
                evidence.push(`=== Context target: ${target} ===\n${step.output}`);
            return { steps, evidence };
        }
        if (stat.isDirectory()) {
            ...
            for (const filePath of candidates) {
                ...
                steps.push(step);
                if (step.success && step.output)
                    evidence.push(`=== Context target: ${filePath} ===\n${step.output}`);
            }
            return { steps, evidence };
        }
...
export async function executeAnalyzeProblem(thought, ego, options) {
    ...
    const filePaths = thought.actionParams?.logPaths ?? [];
    const sourcePaths = thought.actionParams?.sourcePaths ?? [];
    const requiresLocalEvidence = thought.actionParams?.requiresLocalEvidence === true;
    const contextualTargets = [
        ...(thought.actionParams?.localEvidenceTargets ?? []),
    ...
    for (const filePath of filePaths.slice(0, 5)) {
        ...
        steps.push(step);
        if (step.success && step.output)
            gatheredInfo.push(`=== File: ${filePath} ===\n${step.output}`);
    }
    ...
    for (const srcPath of sourcePaths.slice(0, 3)) {
        ...
        steps.push(step);
        if (step.success && step.output)
            gatheredInfo.push(`=== Source: ${srcPath} ===\n${step.output}`);
    }
    ...
    const taskStatus = unmetCriteria.length > 0 ? "failed" : "completed";
...
// ---------------------------------------------------------------------------
async function executeBoundedLocalAgentTask(thought, ego, options) {
    ...
    const status = evidence.length > 0 ? "partial" : "blocked";
...
export async function executeRunAgentTask(thought, ego, options) {
...
async function executeRunAgentTaskViaSubagent(thought, ego, options) {
    const target = resolveTargetProject(ego, thought, options.workspaceContext);
...
Remaining risk or a sensible next improvement.`;
    ...
    const taskStatus = reportStatusToTaskStatus(report);
    await updateEgoStore(resolveEgoStorePath(), (e) => {
        const t = (e.activeTasks ?? []).find((at) => at.id === taskId);
        if (t) {
            t.status = taskStatus;
...
// ---------------------------------------------------------------------------
export async function executeReportFindings(thought, ego, options) {
    const completedTasks = (ego.activeTasks ?? []).filter((t) => isReportableTask(t) && !t.resultDelivered);
...
// ---------------------------------------------------------------------------
function resolveSoulProjectDir() {
 ...
 */
function parseImprovementProposal(text) {
 ...
 */
function sanitizePathCandidate(raw) {
    return raw.trim().replace(/^[`"'(<\[]+|[`"')>\],.;:!?]+$/g, "");
}
function expandPathCandidate(raw) {
...
function gatherTargetCandidates(ego, thought, workspaceContext) {
    const candidates = [];
    const thoughtTexts = [
    ...
    const projectContexts = [...(ego.projectContexts ?? [])].sort((a, b) => {
        if (a.root === ego.activeProjectRoot)
            return -1;
        if (b.root === ego.activeProjectRoot)
            return 1;
        return b.lastObservedAt - a.lastObservedAt;
    });
    for (const context of projectContexts) {
        if (context.confidence >= 0.8)
            candidates.push({ raw: context.root, source: "projectContext" });
    }
...
function explicitlyNamesSoulProject(ego, thought, workspaceContext) {
    const params = thought?.actionParams ?? {};
    const texts = [
        thought?.content,
        thought?.triggerDetail,
...
function resolveTargetProject(ego, thought, workspaceContext) {
...
function getSourceFiles(dir) {
    ...
    const visit = (current, depth) => {
        if (depth > 5 || files.length >= MAX_IMPROVEMENT_SOURCE_FILES)
            return;
        let entries;
        try {
        ...
        for (const entry of entries) {
            if (files.length >= MAX_IMPROVEMENT_SOURCE_FILES)
                break;
            const lower = entry.name.toLowerCase();
            const fullPath = join(current, entry.name);
            if (entry.isDirectory()) {
                if (!IGNORED_SOURCE_DIRS.has(lower))
                    visit(fullPath, depth + 1);
                continue;
            }
            if (!entry.isFile() || PROTECTED_FILES.has(entry.name))
                continue;
...
export async function executeObserveAndImprove(thought, ego, options) {
    if (!options.llmGenerator) {
    ...
    const activeImprove = (ego.activeTasks ?? []).filter((t) => t.status === "in-progress" && t.title?.toLowerCase().includes("improvement")).length;
    if (activeImprove >= 1) {
...
- If nothing clearly needs fixing, set oldCode and newCode to empty strings`;
        ...
        if (parsed.proposal) {
            try {
                ...
                if (!oldCode && !newCode) {
                    ...
                    const applyPatch = (original, old, repl) => {
                        if (original.includes(old))
                            return original.replace(old, repl);
    ...
    const taskStatus = fixApplied || (readOnlyMode && analysisCompleted && !analysisFailure)
        ? "completed"
        : "failed";
...
export async function executeSubagentImprove(thought, ego, options) {
...
async function completeTask(taskId, result, status = "completed", resultDelivered = false) {
    let linkedIntentionId;
...
// Helpers
// ---------------------------------------------------------------------------
function makeSkippedStep(action, reason) {
    return {
        id: randomBytes(4).toString("hex"),
        timestamp: Date.now(),
        action,
...
async function readLocalFile(actionName, filePath) {
...
async function runToolStep(actionName, tool, args, options) {
    const start = Date.now();
    ...
    await updateEgoStore(storePath, (e) => {
        ...
        try {
            ...
            for (const name of readdirSync(resultDir)) {
                ...
                if (knownResultFiles.has(filePath))
                    continue;
                try {
