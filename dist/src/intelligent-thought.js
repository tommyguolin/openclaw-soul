...
function currentConversationMemories(ego, limit = 8) {
    ...
    const hardBoundary = latestInbound.semanticSignals?.some((signal) => signal === "topic-shift" || signal === "closure");
...
function matchesAnyTopic(text, topics) {
    ...
    for (const topic of topics) {
        ...
        if (tokens.length > 0 && tokens.some((t) => t.length >= 2 && (textTokens.has(t) || lower.includes(t))))
            return true;
    }
    return false;
}
function analyzeNeedGaps(needs) {
    const opportunities = [];
...
function analyzeDesires(desires) {
    const opportunities = [];
...
function analyzeRecentInteraction(timeSinceLastInteraction) {
    const opportunities = [];
...
function analyzeMemories(memories) {
    ...
    const hasRecentInteraction = memories.some((m) => m.type === "interaction" && Date.now() - m.timestamp < 2 * 60 * 60 * 1000);
...
function buildUserProfile(ego) {
    ...
    if (projects.length > 0)
        parts.push(`Working on: ${projects.slice(0, 3).join(", ")}`);
    if (skills.length > 0)
        parts.push(`Skills: ${skills.slice(0, 3).join(", ")}`);
    if (interests.length > 0)
        parts.push(`Interests: ${interests.slice(0, 3).join(", ")}`);
    if (habits.length > 0)
        parts.push(`Habits: ${habits.slice(0, 2).join(", ")}`);
    return {
        interests,
        projects,
        skills,
 ...
 */
function isSearchableContent(text) {
    const trimmed = text.trim();
    if (trimmed.length < 10)
        return false;
    const lower = trimmed.toLowerCase();
    // Test / debug messages
    if (/^(测试|test|ping|pong|hello|hi$|hey|收到|好的|ok|确认|嗯|啊|哦|嗨|你好|哈+)/i.test(lower))
        return false;
    if (/测试成功|测试一下|test\s*(success|ok|pass)/i.test(lower))
        return false;
    // Meta-questions about the bot itself
    if (/你(怎么|为什么|为啥).{0,6}(说|做|回答|回复|一直|总是)/i.test(lower))
        return false;
    if (/你(能|可以|会不会|是不是).{0,6}(收到|听到|看到|明白|懂)/i.test(lower))
        return false;
    if (/(why|how).{0,10}(are you|do you|you keep|you always)/i.test(lower))
        return false;
    // Pure exclamations (no question structure)
    if (/^[^?？]*[!！]+$/.test(trimmed))
        return false;
    if (/^[哈嘿哦嗯啊]+[!！~～]*$/.test(trimmed))
        return false;
    // Greetings / small talk
    if (/^(早上?好|晚上?好|早安|晚安|中午好|下午好|good morning|good evening|good night)/i.test(lower))
        return false;
 ...
 */
function isGenuineQuestion(text, semanticSignals = []) {
    if (!isSearchableContent(text))
        return false;
    ...
    if (semanticSignals?.includes("question"))
        return true;
    if (semanticSignals && semanticSignals.length > 0)
        return false;
    const trimmed = text.trim();
    const lower = trimmed.toLowerCase();
    // Must contain a question mark OR a question word
    const hasQuestionMark = /[?？]/.test(trimmed);
    const hasQuestionWord = /^(how|what|why|when|where|who|which|can you|could you|is there|are there|do you know)/i.test(lower) ||
...
function hasKnownLocalEvidenceTarget(ego) {
...
function suppressOrRerouteLowValueMessageThought(thought, opportunity, ctx) {
    if (thought.actionType !== "send-message")
        return;
    const combined = [
        thought.content,
    ...
    if (!isMetaWorkPromiseText(combined))
        return;
...
export function isExecutionFocusedOpportunity(opportunity) {
    const action = opportunity.suggestedAction;
    if (action === "observe-and-improve" ||
...
function hasHandledDirectiveAfter(ego, timestamp) {
    ...
    return (ego.behaviorLog ?? []).some((entry) => executionActions.has(entry.actionType) &&
        entry.timestamp >= timestamp &&
        entry.outcome !== "expired");
}
function evidenceQuestionSignature(text) {
    return text
        .replace(/\s+/g, " ")
        .replace(/[“”"'`]/g, "")
        .trim()
...
function hasLocalEvidenceMissingResultAfter(ego, text, timestamp) {
    const signature = evidenceQuestionSignature(text);
    if (!signature)
        return false;
    return (ego.activeTasks ?? []).some((task) => {
        if ((task.createdAt ?? 0) < timestamp)
            return false;
        if (!/local-evidence-target-missing/i.test(String(task.result ?? "")))
            return false;
        const haystack = [
            task.title,
            task.description,
...
export function hasRecentLocalEvidenceMissingResult(ego, now = Date.now()) {
    const cutoff = now - 2 * 60 * 60 * 1000;
    return (ego.activeTasks ?? []).some((task) => (task.createdAt ?? 0) >= cutoff &&
        /local-evidence-target-missing/i.test(String(task.result ?? "")));
}
export function hasUnresolvedLocalEvidenceMissingResult(ego) {
    const latestMissing = [...(ego.activeTasks ?? [])]
        .filter((task) => /local-evidence-target-missing/i.test(String(task.result ?? "")))
        .sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0))[0];
    if (!latestMissing)
        return false;
    if (hasKnownLocalEvidenceTarget(ego))
        return false;
    const latestInboundAfterMissing = (ego.memories ?? []).some((memory) => memory.type === "interaction" &&
 ...
 */
function isForwardedTaskReport(text) {
...
function analyzeConversationReplay(ctx) {
    const opportunities = [];
    const { ego } = ctx;
    const now = Date.now();
    ...
    const directiveMemories = recentInteractions.filter((m) => m.tags.includes("inbound") &&
    ...
    for (const mem of directiveMemories.slice(0, 2)) {
        ...
        const hasRoutingSemantics = semanticSignals.some((signal) => signal === "local-evidence"
        ...
        const useImprove = !needsLocalEvidence && (semanticSignals.includes("self-improvement")
            || (!hasRoutingSemantics && shouldUseObserveAndImproveForDirective(mem.content)));
        const useAgent = !needsLocalEvidence && !useImprove && (semanticSignals.includes("code-change")
    ...
    for (const qMem of questionMemories.slice(0, 2)) {
        ...
        const hasRelatedKnowledge = qMem.tags.some((tag) => ego.memories.some((m) => isGroundedLearning(m) &&
            m.tags.some((t) => t === tag) &&
    ...
    const substantiveInteractions = recentInteractions.filter((m) => {
        const meaningfulTags = m.tags.filter((t) => t !== "conversation" && t !== "inbound" && t !== "outbound");
    ...
    for (const mem of substantiveInteractions.slice(0, 5)) {
        const meaningfulTags = mem.tags.filter((t) => t !== "conversation" && t !== "inbound" && t !== "outbound");
        // Check if Soul learned something about these tags AFTER the conversation
        const newLearnings = ego.memories.filter((m) => isGroundedLearning(m) &&
        ...
        if (newLearnings.length > 0) {
            ...
            const mentionedProblem = /error|bug|issue|problem|fix|solved|resolve|workaround|solution|问题|解决|修复|办法/i.test(mem.content);
            opportunities.push({
                type: "conversation-replay",
                trigger: "memory",
    ...
    // =====================================================
    const userTopics = [
        ...topicFocus.active.map((topic) => ({ topic, source: "interest" })),
        ...userProfile.projects.map((p) => ({ topic: p, source: "project" })),
        ...userProfile.interests.slice(0, 3).map((i) => ({ topic: i, source: "interest" })),
        ...userProfile.skills.slice(0, 2).map((s) => ({ topic: s, source: "skill" })),
    ...
    for (const { topic, source } of userTopics.slice(0, 3)) {
        ...
        const hasRecentKnowledge = ego.memories.some((m) => isGroundedLearning(m) &&
            m.timestamp > now - 24 * 60 * 60 * 1000 &&
            m.tags.some((t) => topic.toLowerCase().split(/\s+/).some((word) => t.includes(word) && word.length > 2)));
    ...
    if (userProfile.challenges.length > 0) {
        ...
        const hasRecentSearch = ego.memories.some((m) => isGroundedLearning(m) &&
    ...
    if (substantiveInteractions.length > 0) {
        ...
        if (hoursSince >= 1 && hoursSince <= 12) {
            ...
            const meaningfulTags = lastSubstantive.tags.filter((t) => t !== "conversation" && t !== "inbound" && t !== "outbound");
    ...
    if ((userInterests.length > 0 || topicFocus.active.length > 0)
        ...
        const hasRecentPush = ego.memories.some((m) => m.type === "learning" &&
...
function analyzeContextualTriggers(ctx, includeMaintenance = false) {
    const opportunities = [];
    const { ego } = ctx;
    const userFacts = activeUserFacts(ego);
    const localEvidenceBlocked = hasUnresolvedLocalEvidenceMissingResult(ego);
    ...
    if (userFacts.length > 0 && !localEvidenceBlocked) {
        const projectFacts = userFacts.filter((f) => f.category === "project" || f.category === "interest" || f.category === "tech_stack");
    ...
    const hasInteractions = ego.memories.some((m) => m.type === "interaction" && Date.now() - m.timestamp < 24 * 60 * 60 * 1000);
    ...
    if (!localEvidenceBlocked) {
        ...
        if (userFactsForCheckIn.length > 0 && ctx.timeSinceLastInteraction > 5 * 60 * 1000) {
            const recentCheckIn = (ego.behaviorLog ?? []).some((entry) => entry.actionType === "proactive-check-in" && Date.now() - entry.timestamp < 15 * 60 * 1000);
            if (!recentCheckIn) {
                opportunities.push({
                    type: "opportunity-detected",
                    trigger: "need",
    ...
    if (includeMaintenance) {
        ...
        const improvementGoals = ego.goals.filter((g) => g.status === "active" && IMPROVE_RE.test(g.title + g.description));
        // Also check userFacts/userPreferences for self-improvement directives
        const hasImproveFact = activeUserFacts(ego).some((f) => f.confidence >= 0.8 && /优化|observe.*log|自主|self.?improv|proactive.*optim/i.test(f.content));
        const hasImprovePref = (ego.userPreferences ?? []).some((p) => p.confidence >= 0.8 && /优化|improve|self.?improv/i.test(p.preference));
...
export function detectThoughtOpportunities(ctx) {
 ...
 */
export function detectMaintenanceOpportunities(ctx) {
    const maintenance = [
        ...analyzeNeedGaps(ctx.ego.needs),
        ...analyzeContextualTriggers(ctx, true).filter((opportunity) => opportunity.type === "self-improvement-monitor"),
    ];
 ...
 */
export async function llmReRankOpportunities(opportunities, ctx, recentActionHistory, llmGenerator) {
...
function getThoughtContentForOpportunity(opportunity, ego) {
    const need = opportunity.relatedNeeds[0]
        ? ego.needs[opportunity.relatedNeeds[0]]
        : null;
...
function calculateMetricDeltas(opportunity) {
    const deltas = [];
...
export function getActionForOpportunity(opportunity, ego) {
    return determineActionForOpportunity(opportunity, ego);
}
export function buildThoughtFromOpportunity(opportunity, ego) {
...
function isActionableCompletedTaskResult(result) {
...
function isInternalNeedGapOpportunity(opportunity) {
...
function determineActionForOpportunity(opportunity, ego) {
    const completedUndeliveredTasks = (ego.activeTasks ?? []).filter((t) => (t.status === "completed" || t.status === "failed") && !t.resultDelivered && t.result);
    ...
    const pendingFixTask = (ego.activeTasks ?? []).find((t) => t.status === "completed" && !t.resultDelivered && t.result &&
    ...
    const completableFixTasks = (ego.activeTasks ?? []).filter((t) => t.status === "completed" && !t.resultDelivered && t.result &&
    ...
    if (type === "skill-gap" ||
        ...
        if (growthNeed.current < growthNeed.ideal * 0.45
            && opportunity.priority >= 75
            && Math.random() < learnProbability) {
            const isEgoInternal = /need (could improve|critically low|is low)|\d+\/\d+$/.test(opportunity.triggerDetail);
            if (!isEgoInternal) {
                const topics = extractLearningTopics(opportunity.triggerDetail + " " + opportunity.motivation);
                if (topics.length > 0) {
                    return {
...
function extractLearningTopics(text) {
...
function extractExistentialTopics(motivation) {
    const topicMap = [
...
function queryOverlapsThought(query, thoughtContent) {
    const normalizedQuery = normalizeQueryText(query);
    const normalizedThought = normalizeQueryText(thoughtContent);
    if (!normalizedQuery || !normalizedThought)
        return false;
    if (normalizedThought.includes(normalizedQuery) || normalizedQuery.includes(normalizedThought))
        return true;
    const queryTerms = normalizedQuery.split(/\s+/).filter((term) => term.length >= 3);
    if (queryTerms.length === 0)
        return normalizedQuery.length >= 2 && normalizedThought.includes(normalizedQuery);
...
function deriveSearchQueryFromThought(thoughtContent) {
...
async function expandThoughtActionWithAdjacentIdea(thought, opportunity, ctx, llmGenerator) {
    ...
    if (!expandableActions.includes(actionType))
        return;
...
export async function generateIntelligentThought(ctx, options) {
    ...
    if (opportunities.length > 0) {
        const topN = opportunities.slice(0, 5).map((o) => `${o.type}(P=${o.priority},action=${o.suggestedAction ?? "auto"})`).join(", ");
    ...
    if (llmGenerator) {
        try {
            ...
            else if (!isProtectedAction && (selectedOpportunity.type === "skill-gap" ||
                selectedOpportunity.type === "opportunity-detected")) {
                const topics = extractLearningTopics(refinedContent + " " + selectedOpportunity.triggerDetail);
                if (topics.length > 0) {
...
async function generateLLMThoughtPrompt(opportunity, ctx) {
    const { ego } = ctx;
    const needsList = opportunity.relatedNeeds
        .map((key) => {
        const need = ego.needs[key];
...
export async function generateProactiveMessage(ego, llmGenerator) {
...
async function generateProactiveMessagePromptLLM(opportunity, ego) {
