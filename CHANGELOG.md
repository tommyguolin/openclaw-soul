...
- **Carry selected thoughts into proactive delivery**: The message writer now receives the full selected thought and may withhold it only for emptiness, repetition, safety, or unsupported facts; it no longer loses the selected insight and returns `NO_MESSAGE` merely because the message is proactive.
- **Let high-priority connection win over lower-priority research**: Execution protection can no longer displace a higher-ranked relationship or continuity message merely because the lower-ranked candidate would perform research.
- **Do not suppress an unsent follow-up**: Thought-type diversity filtering now keeps `send-message` candidates eligible until ordinary delivery gates make a real decision, rather than treating a previously generated but undelivered bond thought as already expressed.
- **Preserve ready conversation candidates through routing**: Service-level selection now retains a ready, equally or higher-ranked `send-message` opportunity after diversity and execution filtering, so background research cannot make Soul silently skip a legitimate proactive follow-up.
- **Let Soul develop complete thoughts**: Ordinary, private, proactive, research, recall, and self-reflection model paths no longer impose sentence/character truncation; all opportunity priorities—including search, research, content-push, and other execution-focused opportunities—use the configured thought model, while action routing and permissions remain separate. Gateway/OpenAI-compatible calls omit a completion ceiling unless `maxTokens` is explicitly configured, and Thought Pool/ThoughtEpisode persistence retain the complete generated content.
